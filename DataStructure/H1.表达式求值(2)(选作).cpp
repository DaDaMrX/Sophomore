...
int calculate(char s[])
{
	...
	while (i < len)
	{
		...
		{
			if (i == 0 || !isdigit(s[i - 1]) && !isalpha(s[i - 1]) && s[i - 1] != ')') throw Error("error.");
			back_to('(', num, op, '#');
			op.pop();
			i++;
		}
		else if (s[i] == '-' && (i == 0 || !isdigit(s[i - 1]) && !isalpha(s[i - 1]) && s[i - 1] != ')'))
		{
			sign = -1;
			i++;
		}
